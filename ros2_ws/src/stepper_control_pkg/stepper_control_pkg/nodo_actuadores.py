...
Conversión grados → pasos:
    ...
    Con pasos_por_rev=200 (paso completo):
        90°  →  50 pasos
        45°  →  25 pasos
        1°   →    0.56 pasos ≈ 1 pasos
...
class NodoActuadores(Node):
    ...
    Parámetros ROS 2:
        ...
        pasos_por_rev(int)   : pasos/revolución incluyendo microstepping [default: 200]
        delay_pulso  (float) : semi-ciclo del pulso STEP en segundos     [default: 0.002]
                               → Periodo completo = 2 × delay_pulso
                               → 0.002 s → 4 ms/ciclo → 250 pasos/s → ≈75 RPM
    """
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("pasos_por_rev", 200)
        self.declare_parameter("delay_pulso",   0.002)
    ...
    def _cb_comando_grados(self, msg: Float32) -> None:
        ...
        Ejemplos con pasos_por_rev=200:
            +90.0°  →  +50 pasos  (CW)
            -45.5°  →  -25 pasos  (CCW)
            +360.0° → +200 pasos  (1 vuelta completa CW)
               0.0° →    0 pasos   (stop)
        """
        grados = msg.data
        pasos_por_rev: int = (
